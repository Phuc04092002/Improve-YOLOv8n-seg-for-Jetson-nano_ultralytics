...
class GCDetectHead(nn.Module):
    def __init__(self, in_channels, num_classes, num_anchors=1, num_groups=16):
        ...
        self.box_pred = nn.Conv2d(in_channels, 4 * num_anchors, kernel_size=1)

    def forward(self, x):
        # Shared feature extraction
        x = self.act1(self.bn1(self.group_conv1(x)))
        x = self.act2(self.bn2(self.group_conv2(x)))

        # Output branches
        cls_out = self.cls_pred(x)  # [B, num_classes*num_anchors, H, W]
        box_out = self.box_pred(x)  # [B, 4*num_anchors, H, W]
